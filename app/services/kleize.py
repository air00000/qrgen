...
def create_kleize_image(nazvanie: str, price: float, photo: str, url: str) -> bytes:
    ...
    try:
        ...
        rebond_semibold = ImageFont.truetype(os.path.join(fonts_dir, "RebondGrotesqueSemibold.ttf"),
                                             int(48 * SCALE_FACTOR))
        ...
        if photo:
            ...
            if USE_FIXED_COORDS:
                ...
                px = int(
                    (nodes['pic']['absoluteBoundingBox']['x'] - frame_node['absoluteBoundingBox']['x']) * SCALE_FACTOR)
                py = int(
                    (nodes['pic']['absoluteBoundingBox']['y'] - frame_node['absoluteBoundingBox']['y']) * SCALE_FACTOR)
        ...
        if nodes['qr'] and 'absoluteBoundingBox' in nodes['qr']:
            ...
            if qr_img:
                ...
                qx = int(
                    (nodes['qr']['absoluteBoundingBox']['x'] - frame_node['absoluteBoundingBox']['x']) * SCALE_FACTOR)
                qy = int(
                    (nodes['qr']['absoluteBoundingBox']['y'] - frame_node['absoluteBoundingBox']['y']) * SCALE_FACTOR)
        ...
        if nodes['nazvanie']:
            ...
            ny = (nodes['nazvanie']['absoluteBoundingBox']['y'] - frame_node['absoluteBoundingBox'][
                'y']) * SCALE_FACTOR + offset
        ...
        if nodes['price']:
            ...
            py = (nodes['price']['absoluteBoundingBox']['y'] - frame_node['absoluteBoundingBox'][
                'y']) * SCALE_FACTOR + offset
        ...
        if nodes['time']:
            ...
            ty = (nodes['time']['absoluteBoundingBox']['y'] - frame_node['absoluteBoundingBox'][
                'y']) * SCALE_FACTOR + offset
