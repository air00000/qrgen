...
## Утилита калибровки размеров текста (Figma → Rust)

Чтобы приблизить рендер текста в Rust к размерам текста в Figma, добавлена утилита:
...
cargo run -p qrgen-backend --bin text_calibrate -- \
  ...
  --spacing 0.0 --figma-px 56
```

Что делает:

- перебирает `px` в заданном диапазоне (`--min/--max/--step`)
- рендерит текст тем же `rusttype`
- измеряет фактический bbox (ширина/высота)
- возвращает `best_px` и коэффициент `figma_to_rust_ratio`

Это не магический «1 в 1» для всех случаев, но даёт воспроизводимый способ подбора размера под конкретные шрифты/веса/строки.
